...
export default function RatingModal({
  ...
  useEffect(() => {
    ...
    setLoaded(false);
    (async () => {
      const existing = await fetchMyRating(fromUserId, toUserId);
      if (existing) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await submitRating({ fromUserId, toUserId, rating, comment });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="text-lg font-bold mb-1">Noter {toUserName}</h3>
        <p className="text-xs text-[var(--loboko-text-muted)] mb-5">
          Donnez votre avis sur ce prestataire
        </p>
