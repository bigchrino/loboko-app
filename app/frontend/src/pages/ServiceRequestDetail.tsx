...
/**
 * ServiceRequestDetail — page /requests/:id
 *
 * Shows the request body, its responses, and — for providers — a reply
 * form. Owner can close / reopen / delete and open a direct chat with
 * any responder.
 */
...
export default function ServiceRequestDetail() {
  ...
  return (
    <Layout>
      ...
      {/* Reply form for providers (not the owner) */}
      {user && !isOwner && request.status === 'open' && (
        <section className="mt-4 p-4 rounded-2xl bg-[var(--loboko-surface)] border border-[var(--loboko-border)]">
          ...
          {!isProvider && (
            <p className="text-xs text-[var(--loboko-text-muted)] mb-2">
              Les prestataires peuvent répondre ; les clients peuvent aussi laisser un commentaire.
            </p>
          )}
          <textarea
            value={message}
